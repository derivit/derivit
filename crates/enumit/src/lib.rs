              ...
              match &f.ident {
                Some(name) => {
                  let name = field_opts
                    ...
                    .as_ref()
                    .cloned()
                    .unwrap_or_else(|| format_ident!("{}", name.to_string().to_pascal_case()));
                  let ty = field_opts
                    .ty
                    .as_ref()
                  ...
                  let display = field_opts
                    .display
                    .clone()
                    .unwrap_or_else(|| match opts.display {
                      ...
                      Case::CamelCase => name.to_string().to_lower_camel_case(),
                      Case::SnakeCase => name.to_string().to_snake_case(),
                      Case::ScreamingSnakeCase => name.to_string().to_shouty_snake_case(),
                      Case::KebabCase => name.to_string().to_kebab_case(),
                      Case::ScreamingKebabCase => name.to_string().to_shouty_kebab_case(),
                ...
                None => {
                  return syn::Error::new_spanned(f.ident, "Unnamed fields are not supported")
                    .to_compile_error()
                    .into()
                }
              }
          ...
          match iter.next().unwrap() {
            TokenTree::Ident(name) => match name.to_string().as_str() {
              "name" => {
                ...
                field_name = Some(syn::parse_str(&parse_str_val(&name.span(), &mut iter)?)?);
              }
              "display" => {
                if display.is_some() {
                  return Err(syn::Error::new_spanned(
              ...
              "attrs" => {
                let stream = (&mut iter).take_while(
                                ...
                                .collect::<proc_macro2::TokenStream>();
                if stream.is_empty() {
                  return Err(syn::Error::new_spanned(name, "attrs must have a value"));
                }

                let g = match stream.into_iter().next() {
                  Some(TokenTree::Group(g)) if g.delimiter() == Delimiter::Parenthesis => Ok(g),
                  _ => Err(syn::Error::new_spanned(
                    attr,
...
fn parse_str_val(span: &Span, iter: &mut dyn Iterator<Item = TokenTree>) -> syn::Result<String> {
  ...
  let lit = iter
    ...
    .to_string();
  Ok(
    lit[usize::from(lit.starts_with('"'))..lit.len() - usize::from(lit.ends_with('"'))].to_string(),
  )
}
        ...
        for nested in list.nested {
          if let syn::NestedMeta::Meta(meta) = nested {
            match () {
                ...
                if let syn::Meta::NameValue(name_value) = meta {
                  if let syn::Lit::Str(s) = name_value.lit {
                    ...
                    match s.value().as_str() {
                      ...
                      "SCREAMING_SNAKE_CASE" => opts.display = Case::ScreamingSnakeCase,
                      "kebab-case" => opts.display = Case::KebabCase,
                      "SCREAMING-KEBAB-CASE" => opts.display = Case::ScreamingKebabCase,
                    ...
                    return Err(syn::Error::new_spanned(
                      name_value.lit,
                      "The display_all attribute value must be a string literal",
                    ));
                  }
                } else {
                  return Err(syn::Error::new_spanned(
              ...
              () if meta.path().is_ident("typed") => match has_is_typed {
                Some(v) => {
                  return Err(syn::Error::new_spanned(
                    meta,
                    format!("Enum has been set to be {}", v),
                  ...
                  match meta {
                    ...
                    Meta::List(l) => {
                      opts.ty = Some(parse_str::<syn::Type>(
                        &syn::parse2::<syn::LitStr>(l.nested.to_token_stream())?.value(),
                      )?);
                    }
                    Meta::NameValue(_) => {
                      return Err(syn::Error::new_spanned(
                        meta,
                        "The typed attribute cannot be a name value pair",
                      ));
                    }
                  }
                    ...
                    if let syn::NestedMeta::Meta(meta) = nested {
                      match () {
                        () if meta.path().is_ident("start") => {
                          if let syn::Meta::NameValue(name_value) = meta {
                            if let syn::Lit::Int(s) = &name_value.lit {
                              ...
                              return Err(syn::Error::new_spanned(
                                &name_value.lit,
                                "The name attribute value must be a int literal",
                              ));
                            }
                          } else {
                            return Err(syn::Error::new_spanned(
                        ...
                        () if meta.path().is_ident("type") => {
                          if let syn::Meta::NameValue(name_value) = meta {
                            if let syn::Lit::Str(s) = &name_value.lit {
                              ty = Some(parse_str::<syn::Ident>(&s.value())?);
                            } else {
                              return Err(syn::Error::new_spanned(
                                &name_value.lit,
                                "The name attribute value must be a string literal",
              ...
              () if meta.path().is_ident("attrs") => {
                if let syn::Meta::List(list) = meta {
                  for n in &list.nested {
                    opts
                      .attrs
                      .extend(Attribute::parse_outer.parse2(quote::quote! {#[#n]})?);
                  }
                } else {
                  return Err(syn::Error::new_spanned(
                    meta,
