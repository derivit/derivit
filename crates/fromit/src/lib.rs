    ...
    Some(b) => {
      ...
      if b.inherit {
        ...
        if ts.is_empty() {
          ...
          if !self_bound.to_token_stream().is_empty() {
                ...
                for item in items {
                  match item {
                    syn::NestedMeta::Meta(inner) => {
                      ...
                      match name.as_str() {
                        "name" => {
                          derivit_core::parser::Parser::parse(&name, inner, &mut struct_name)?
                        }
                        "extra" => derivit_core::parser::Parser::parse(&name, inner, &mut extra)?,
                        "attributes" => {
                          derivit_core::parser::Parser::parse(&name, inner, &mut attributes)?
                        }
                        "getters" => {
                          derivit_core::parser::Parser::parse(&name, inner, &mut getters)?
                        }
                        "setters" => {
                          derivit_core::parser::Parser::parse(&name, inner, &mut setters)?
                        }
                        "converter" => {
                          derivit_core::parser::Parser::parse(&name, inner, &mut converter)?
                        }
                        "vis" => derivit_core::parser::Parser::parse(&name, inner, &mut vis)?,
                        "bound" => derivit_core::parser::Parser::parse(&name, inner, &mut bound)?,
          ...
          for attr in &field.attrs {
            match ToString::to_string(&attr.path.clone().into_token_stream()).as_str() {
              "fromit" => {
                ...
                let mut getter: (bool, Option<FieldGetterOptions>) = (false, None);
                let mut setter: (bool, Option<FieldSetterOptions>) = (false, None);
                      ...
                      match *item {
                        syn::NestedMeta::Meta(ref inner) => {
                          ...
                          match name.as_str() {
                            ...
                            "skip" => derivit_core::parser::Parser::parse(&name, inner, &mut skip)?,
                            "type" => derivit_core::parser::Parser::parse(&name, inner, &mut typ)?,
                            "rename" => {
                              derivit_core::parser::Parser::parse(&name, inner, &mut rename)?
                            }
                            "parent" => {
                              derivit_core::parser::Parser::parse(&name, inner, &mut parent)?
                            }
                            "from" => derivit_core::parser::Parser::parse(&name, inner, &mut from)?,
                            "into" => derivit_core::parser::Parser::parse(&name, inner, &mut into)?,
                            "getter" => {
                              derivit_core::parser::Parser::parse(&name, inner, &mut getter)?
                            }
                            "setter" => {
                              derivit_core::parser::Parser::parse(&name, inner, &mut setter)?
                            }
                            "vis" => derivit_core::parser::Parser::parse(&name, inner, &mut vis)?,
                            "attributes" => {
                              derivit_core::parser::Parser::parse(&name, inner, &mut attributes)?
...
pub fn from_it(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
  ...
  for (name, opts) in fromit.struct_opts {
    ...
    structure.push(match generate_struct(&name, &opts, &final_generics) {
      Ok(s) => s,
      Err(e) => return e.to_compile_error().into(),
    });

    structure.push(match generate_from(src_name, &opts, &final_generics) {
      Ok(s) => s,
      Err(e) => return e.to_compile_error().into(),
    });

    structure.push(match generate_into(src_name, &opts, &final_generics) {
      Ok(s) => s,
      Err(e) => return e.to_compile_error().into(),
    });

    structure.push(match generate_getters(&opts, &final_generics) {
      Ok(s) => s,
      Err(e) => return e.to_compile_error().into(),
    });

    structure.push(match generate_setters(&opts, &final_generics) {
      Ok(s) => s,
      Err(e) => return e.to_compile_error().into(),
    });
