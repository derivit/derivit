use derivit_core::setter::{FieldSetterOptions, StructSetterOptions};
              ...
              if !extra_fields.0 {
                match inner {
                  ...
                  syn::Meta::List(l) => {
                    extra_fields = (
                      true,
                      Some(derivit_core::parser::Parser::parse_fields(
                        &inner,
        ...
        for (idx, field) in extra_fields.into_iter().enumerate() {
                    ...
                    for item in items {
                      match *item {
                        syn::NestedMeta::Meta(ref inner) => {
                          ...
                          match name.as_str() {
                            "default" => {
                              derivit_core::parser::Parser::parse(&name, inner, &mut default)?
                            }
                            "getter" => {
          ...
          if let std::collections::hash_map::Entry::Vacant(e) = fields.entry(key) {
            e.insert(ExtraField {
              ...
              src_vis: field.vis.clone(),
              vis: None,
              getter: FieldGetterOptions::default(),
              setter: FieldSetterOptions::default(),
              default: None,
...
pub(crate) struct StructOpts {
  ...
  pub(crate) getters: StructGetterOptions,
  pub(crate) setters: StructSetterOptions,
  pub(crate) converter: Converter,
  pub(crate) attributes: Attributes,
  pub(crate) fields: HashMap<String, Field>,
  pub(crate) extra: Option<Extra>,
  pub(crate) debug: Option<derivit_core::Debug>,
}
